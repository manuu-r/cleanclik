...
void main() {
  group('Database Service Tests', () {
    ...
    test('DatabaseResult should handle transformation errors', () {
      final result = DatabaseResult.success(5);
      final transformed = result.map<String>(
    ...
    test('Database services consolidated into DataService', () {
      // All database services have been consolidated into DataService:
      // - InventoryDatabaseService removed - inventory operations now handled by DataService
      // - AchievementDatabaseService removed - achievement operations now handled by DataService
      // - CategoryStatsDatabaseService removed - category stats operations now handled by DataService
      // - LeaderboardDatabaseService removed - leaderboard operations now handled by DataService

      // This test verifies that the consolidation is complete
      expect(true, true);
    });
  });
}
