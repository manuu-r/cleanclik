...
class DatabaseResponses {
  ...
  static PostgrestResponse<T> createSuccessResponse<T>({
    ...
    return PostgrestResponse<T>(data: data, count: count);
  }
  ...
  static PostgrestException createDatabaseError({
    ...
    return PostgrestException(message: message, code: code, details: details);
  }
  ...
  static Map<String, dynamic> createInventoryResponses() {
    return {
      ...
      'delete_inventory_item': createSuccessResponse(data: null),
      ...
      'select_leaderboard': createSuccessResponse(
        data: List.generate(
          ...
          (index) => {
            'user_id': 'user-$index',
            'username': 'user$index',
            // 'full_name': 'User $index', // Removed - column doesn't exist in database schema
            'total_points': 1000 - (index * 100),
            'items_collected': 100 - (index * 10),
            'rank': index + 1,
            'avatar_url': index % 3 == 0
  ...
  static Map<String, dynamic> createRPCResponses() {
    return {
      'calculate_user_rank': createSuccessResponse(
        data: {'rank': 5, 'total_users': 1000, 'percentile': 95.0},
      ),
      ...
      'update_leaderboard': createSuccessResponse(
        data: {'updated': true, 'new_rank': 4, 'points_added': 50},
      ),
    };
  }
  ...
  static Map<String, dynamic> createBatchResponses() {
    return {
      ...
      'batch_delete_items': createSuccessResponse(data: null, count: 3),
    };
  }
  ...
  static Map<String, dynamic> createConfigurationResponses() {
    return {
      ...
      'category_config': createSuccessResponse(
        data: TestConfig.wasteCategories.entries
            .map(
              (entry) => {
                'category': entry.key,
  ...
  static Map<String, dynamic> createTestScenarios() {
    return {
      'offline_sync_scenario': {
        ...
        'conflicts': MockInventoryItems.createConflictedInventoryItems(
          count: 2,
        ),
        'sync_result': {'synced': 6, 'conflicts': 2, 'failed': 0},
      },
      ...
      'real_time_updates_scenario': {
        'leaderboard_stream': List.generate(
          10,
          (index) => {
            'event_type': 'UPDATE',
        ...
        'inventory_stream': List.generate(
          5,
          (index) => {
            'event_type': 'INSERT',
            'timestamp': DateTime.now()
    ...
    return {
      ...
      'aggregation_query': createSuccessResponse(
        data: {
          ...
          'category_totals': TestConfig.wasteCategories.keys
              .map(
                (category) => {
                  'category': category,
                  'total_items': 400000 + (category.hashCode % 100000),
                  'total_points':
                      (400000 + (category.hashCode % 100000)) *
                      (TestConfig.categoryPoints[category] ?? 5),
                },
              )
              .toList(),
          'daily_stats': List.generate(
            30,
            (day) => {
              'date': DateTime.now()
