...
class AuthResponses {
  ...
  static AuthResponse createSignInResponse({Map<String, dynamic>? userData}) {
  ...
  static AuthResponse createSignUpResponse({
    ...
    return AuthResponse(
      ...
      session: emailConfirmationRequired
          ? null
          : Session.fromJson({
              'access_token':
                  'mock-access-token-${DateTime.now().millisecondsSinceEpoch}',
  ...
  static AuthResponse createGoogleSignInResponse() {
    ...
    return AuthResponse(
      ...
      session: Session.fromJson({
        'access_token':
            'mock-google-access-token-${DateTime.now().millisecondsSinceEpoch}',
        'refresh_token':
            'mock-google-refresh-token-${DateTime.now().millisecondsSinceEpoch}',
        'expires_in': 3600,
        'expires_at':
            DateTime.now()
                .add(const Duration(hours: 1))
  ...
  static Map<String, AuthException> getCommonAuthErrors() {
    return {
      ...
      'user_not_found': AuthException('User not found', statusCode: '400'),
  ...
  static AuthResponse createSessionRefreshResponse({
    ...
    return AuthResponse(
      ...
      session: Session.fromJson({
        'access_token':
            'mock-refreshed-access-token-${DateTime.now().millisecondsSinceEpoch}',
        'refresh_token':
            'mock-refreshed-refresh-token-${DateTime.now().millisecondsSinceEpoch}',
        'expires_in': 3600,
        'expires_at':
            DateTime.now()
                .add(const Duration(hours: 1))
  ...
  static AuthResponse createGoogleAuthResponse() {
    final user = MockUsers.createGoogleSignInUser();
