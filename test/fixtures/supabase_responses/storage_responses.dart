...
class StorageResponses {
  ...
  static Map<String, dynamic> createSignedUrlResponse({
    ...
    return {
      'signedUrl':
          'https://test.supabase.co/storage/v1/object/sign/$bucketId/$fileName?token=mock-signed-token',
      'path': '$bucketId/$fileName',
      'expiresIn': expiresIn,
  ...
  static Map<String, dynamic> createStorageAnalyticsResponses() {
    return {
      'bucket_usage': {
        ...
        'fileTypes': {'image/jpeg': 800, 'image/png': 350, 'image/webp': 100},
        'uploadTrends': List.generate(
          30,
          (day) => {
            'date': DateTime.now()
      ...
      'user_storage_usage': {
        ...
        'fileBreakdown': {'images': 40, 'documents': 3, 'videos': 2},
        'recentUploads': List.generate(
          5,
          (index) => {
  ...
  static Map<String, StorageException> getCommonStorageErrors() {
    return {
      'file_not_found': StorageException('File not found', statusCode: '404'),
  ...
  static Map<String, dynamic> createBatchUploadResponse({
    ...
    return {
      ...
      'successful': fileNames
          .map(
            (fileName) =>
                createUploadResponse(fileName: fileName, bucketId: bucketId),
          )
          .toList(),
  ...
  static Map<String, dynamic> createFileMetadataResponse({
    ...
    return {
      ...
      'metadata': {
        ...
        'contentHash':
            'sha256:mock-hash-${DateTime.now().millisecondsSinceEpoch}',
      },
      'customMetadata': {
...
class StorageException implements Exception {
  ...
  const StorageException(this.message, {this.statusCode, this.error});
