...
class MockAuthStates {
  ...
  static Map<String, dynamic> getSignedInState({User? customUser}) {
    if (customUser != null) {
  ...
  static Map<String, dynamic> getAuthErrorState({String? customError}) {
    if (customError != null) {
      return TestDataFactory.createMockAuthState(
        status: 'error',
        user: null,
  ...
  static Map<String, dynamic> getLoadingState() {
    return TestDataFactory.createMockAuthState(status: 'loading', user: null);
  }
  ...
  static List<Map<String, dynamic>> createAuthFlowSequence({
    ...
    switch (flowType) {
      ...
      case 'email_signin':
        return [getSignedOutState(), getLoadingState(), getSignedInState()];
      ...
      case 'signout':
        return [getSignedInState(), getLoadingState(), getSignedOutState()];
  ...
  static Map<String, dynamic> createAuthStateWithProfile({
    ...
    return TestDataFactory.createMockAuthState(status: 'signed_in', user: user);
  }
  ...
  static Map<String, bool Function(Map<String, dynamic>)>
  getValidationHelpers() {
    return {
      'isSignedOut': (state) =>
          state['status'] == 'signed_out' && state['user'] == null,
      'isSignedIn': (state) =>
          state['status'] == 'signed_in' && state['user'] != null,

      'isLoading': (state) => state['status'] == 'loading',
      'hasError': (state) =>
          state['status'] == 'error' && state['error'] != null,
      'needsEmailVerification': (state) =>
          state['status'] == 'email_verification_pending',
      'isGoogleUser': (state) {
      ...
      'isNewUser': (state) {
        ...
        return user['totalPoints'] == 0 &&
            (user['achievements'] as List?)?.isEmpty == true;
      },
      'isPremiumUser': (state) {
        final user = state['user'];
        if (user == null) return false;
  ...
  static Stream<Map<String, dynamic>> createAuthStateStream({
    ...
    final sequence =
        stateSequence ??
        [getSignedOutState(), getLoadingState(), getSignedInState()];
  ...
  static Map<String, Map<String, dynamic>> getErrorStates() {
    return {
      'invalid_credentials': getAuthErrorState(
        customError: 'Invalid email or password',
      ),
      'email_not_confirmed': getAuthErrorState(
        customError: 'Email address not confirmed',
      ),
      'account_locked': getAuthErrorState(
        customError: 'Account temporarily locked',
      ),
      'weak_password': getAuthErrorState(customError: 'Password is too weak'),
      'email_already_exists': getAuthErrorState(
        customError: 'Email address already registered',
      ),
      'network_timeout': getNetworkErrorState(),
      'server_error': getAuthErrorState(
        customError: 'Server temporarily unavailable',
      ),
