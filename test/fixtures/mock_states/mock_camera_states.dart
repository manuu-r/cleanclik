...
class MockCameraStates {
  ...
  static List<Map<String, dynamic>> createCameraFlowSequence({
    ...
    switch (flowType) {
      case 'initialization':
        return [getInitializingState(mode: mode), getReadyState(mode: mode)];

      case 'start_detection':
        return [getReadyState(mode: mode), getDetectingState(mode: mode)];

      case 'stop_detection':
        return [getDetectingState(mode: mode), getReadyState(mode: mode)];
  ...
  getValidationHelpers() {
    return {
      ...
      'isReady': (state) =>
          state['status'] == 'ready' && state['isInitialized'] == true,
  ...
  static Map<String, dynamic> createCameraConfiguration({
    ...
    final deviceState =
        getDeviceCapabilityStates()[deviceType + '_device'] ??
        getDeviceCapabilityStates()['mid_range_device']!;
    final lightingState =
        getLightingConditionStates()[lightingCondition + '_lighting'] ??
    ...
    return {
      ...
      'settings': {
        ...
        'detectionFrequency':
            lightingState['lighting']['detectionAccuracy'] > 0.8
            ? 'high'
            : 'medium',
      },
    };
