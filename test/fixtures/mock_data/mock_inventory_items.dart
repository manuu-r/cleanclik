...
class MockInventoryItems {
  ...
  static List<Map<String, dynamic>> createMockInventoryItems({
    ...
    return List.generate(count, (index) {
      ...
      final labels =
          TestConfig.wasteCategories[category.name] ?? ['Unknown Item'];
  ...
  static List<Map<String, dynamic>> createConflictedInventoryItems({
    ...
    return List.generate(count, (index) {
      ...
      return {
        ...
        'remoteVersion': 3,
        'conflictData': {
          'local': {'label': 'Local Label $index', 'points': 10},
          'remote': {'label': 'Remote Label $index', 'points': 15},
        },
      };
    });
  }
  ...
  static Map<String, dynamic> createInventorySummary({String? userId}) {
    return {
      ...
      'totalPoints': 1250,
      'categoryBreakdown': {
        'recycle': {'items': 50, 'points': 500, 'pending': 5, 'disposed': 45},
        'organic': {'items': 40, 'points': 320, 'pending': 4, 'disposed': 36},
        'landfill': {'items': 25, 'points': 125, 'pending': 3, 'disposed': 22},
        'ewaste': {'items': 8, 'points': 120, 'pending': 2, 'disposed': 6},
        'hazardous': {'items': 2, 'points': 40, 'pending': 1, 'disposed': 1},
      },
  ...
  static Map<String, dynamic> createBatchOperation({
    ...
    return {
      ...
      'estimatedCompletion': DateTime.now()
          ...
          .toIso8601String(),
      'progress': {'total': itemCount, 'completed': 0, 'failed': 0},
    };
  }
  ...
  static Map<String, dynamic> createInventoryAnalytics({String? userId}) {
    return {
      'userId': userId ?? 'test-user-id',
      'period': 'last_30_days',
      'metrics': {
