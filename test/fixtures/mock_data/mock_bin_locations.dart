...
class MockBinLocations {
  ...
  static List<Map<String, dynamic>> createStatusVariedBinLocations() {
    return [
      ...
      // Maintenance bin
      {
        ...
        'estimatedRepairTime': DateTime.now()
  ...
  static Map<String, dynamic> createBinLocationAnalytics({String? binId}) {
    return {
      ...
      'waypoints': bins
          .map(
            (binId) => {
              'binId': binId,
              'latitude':
                  TestConfig.testLatitude + (Random().nextDouble() * 0.01),
              'longitude':
                  TestConfig.testLongitude + (Random().nextDouble() * 0.01),
              'estimatedArrival': DateTime.now()
                  .add(Duration(minutes: bins.indexOf(binId) * 5))
                  .toIso8601String(),
