...
class MockUsers {
  ...
  static Map<String, dynamic> createGoogleSignInUser() {
    return {
      ...
      'metadata': {'signInMethod': 'google', 'emailVerified': true},
    };
  }

  /// Create mock user preferences
  static Map<String, dynamic> createUserPreferences({String? userId}) {
    return {
      ...
      'preferences': {
        ...
        'app': {'theme': 'system', 'language': 'en', 'units': 'metric'},
        'camera': {
          'autoFocus': true,
          'flashMode': 'auto',
          'soundEnabled': true,
  ...
  static Map<String, dynamic> createUserStatistics({String? userId}) {
    return {
      'userId': userId ?? 'test-user-id',
      'statistics': {
        'totalPoints': 1250,
  ...
  static Map<String, dynamic> createNotificationSettings({String? userId}) {
    return {
      'userId': userId ?? 'test-user-id',
      'pushNotifications': {
        'enabled': true,
  ...
  static Map<String, dynamic> createPrivacySettings({String? userId}) {
    return {
      'userId': userId ?? 'test-user-id',
  ...
  static Map<String, dynamic> createDeviceInfo({String? userId}) {
    return {
      'userId': userId ?? 'test-user-id',
      'devices': [
        {
  ...
  static Map<String, dynamic> createSocialConnections({String? userId}) {
    return {
      'userId': userId ?? 'test-user-id',
      ...
      'activities': List.generate(
        ...
        (day) => {
          'date': DateTime.now()
              ...
              .toIso8601String()
              .split('T')[0],
          'itemsCollected': day < 7
              ? (3 + (day % 4))
              : (day % 5), // More active in recent days
