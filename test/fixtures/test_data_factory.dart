...
// All models are now imported from grouped files above
...
class TestDataFactory {
  ...
  static DetectedObject createMockDetectedObject({
    String? trackingId,
    ...
    return DetectedObject(
      trackingId:
          trackingId ?? 'detected-${DateTime.now().millisecondsSinceEpoch}',
  ...
  static BinLocation createMockBinLocation({
    ...
    return BinLocation(
      ...
      metadata:
          metadata ?? {'description': 'A test recycling bin for unit testing'},
  ...
  /// TODO: LeaderboardEntry model needs to be created in lib/core/models/social_models.dart
  static Map<String, dynamic> createMockLeaderboardEntry({
    String? id,
    String? username,
    String? avatarUrl,
    int? totalPoints,
  ...
  static List<DetectedObject> createMockDetectedObjects({
    int count = 3,
    WasteCategory? category,
  }) {
    return List.generate(count, (index) {
      return createMockDetectedObject(
        ...
        category:
            category ??
            WasteCategory.values[index % WasteCategory.values.length],
  ...
  static List<Map<String, dynamic>> createMockLeaderboardEntries({int count = 10}) {
    return List.generate(count, (index) {
  ...
  static List<Map<String, dynamic>> createMockMLKitResults({
    ...
    return List.generate(count, (index) {
      final category =
          WasteCategory.values[index % WasteCategory.values.length];
      final labels =
          TestConfig.wasteCategories[category.name] ?? ['Unknown Object'];
        ...
        'labels': [
          {
            ...
            'confidence':
                minConfidence + (Random().nextDouble() * (1.0 - minConfidence)),
          },
        ],
        'boundingBox': {
          'left': 100.0 + (index * 50),
  ...
  static Map<String, Map<String, dynamic>> createAuthStateFixtures() {
    return {
      'signed_out': createMockAuthState(status: 'signed_out', user: null),
    ...
    return {
      ...
      'large_datasets': [
        ...
        // Large leaderboard (10000 users)
        {
          ...
          'data': List.generate(
            10000,
            (index) => createMockLeaderboardEntry(
  ...
  static Map<String, List<Map<String, dynamic>>> createPerformanceTestData() {
    return {
      'ml_detection_performance': List.generate(
        TestConfig.performanceTestIterations,
        (index) => {
          'iteration': index,
          'objects': createMockDetectedObjects(
            count: Random().nextInt(TestConfig.maxSimultaneousObjects) + 1,
          ),
          'processingTime': Random().nextInt(
            TestConfig.mlProcessingThreshold.inMilliseconds,
          ),
          'memoryUsage': Random().nextInt(TestConfig.memoryUsageThreshold),
        },
      ),
      'camera_switching_performance': List.generate(
        50,
        (index) => {
          'iteration': index,
          'fromMode': CameraMode.values[index % 2],
          'toMode': CameraMode.values[(index + 1) % 2],
          'switchingTime': Random().nextInt(
            TestConfig.cameraSwitchingThreshold.inMilliseconds,
          ),
        },
      ),
      'supabase_sync_performance': List.generate(
        100,
        (index) => {
          'iteration': index,
          'itemCount': Random().nextInt(50) + 1,
          'syncTime': Random().nextInt(
            TestConfig.supabaseSyncThreshold.inMilliseconds,
          ),
