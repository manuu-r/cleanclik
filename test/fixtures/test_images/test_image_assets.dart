...
class TestImageAssets {
  ...
  static Future<Uint8List> loadImageBytes(String imageKey) async {
    ...
    try {
      ...
      bytes = (await rootBundle.load(
        'assets/test_images/${TestImageMetadata.getImageMetadata(imageKey)!['fileName']}',
      )).buffer.asUint8List();
    } catch (e) {
  ...
  static List<String> getEdgeCaseTestImages({String? edgeCaseType}) {
  ...
  static List<String> getQRCodeTestImages({bool includeEdgeCases = true}) {
    var images = TestImageMetadata.getQRCodeImages();

    if (!includeEdgeCases) {
      images = images
          .where((key) => !TestImageMetadata.isEdgeCase(key))
          .toList();
    }
  ...
  static Map<String, dynamic> getImageStatistics() {
    ...
    for (final key in allKeys) {
      ...
      if (TestImageMetadata.isEdgeCase(key)) {
        final edgeCaseType =
            TestImageMetadata.getEdgeCaseType(key) ?? 'unknown';
    ...
    return {
      ...
      'averageFileSizeKB': ((totalSize / allKeys.length) / 1024)
          .toStringAsFixed(1),
    };
  }
  ...
  static Future<Uint8List> _generatePlaceholderImage(String imageKey) async {
    ...
    if (metadata == null) {
      throw ArgumentError(
        'Cannot generate placeholder: metadata not found for $imageKey',
      );
    }
  ...
  static Map<String, List<String>> createTestImageSets() {
    return {
      ...
      'difficulty_very_hard': TestImageMetadata.getImagesByDifficulty(
        'very_hard',
      ),
    };
  }
