...
void main() {
  group('Testing Infrastructure', () {
    ...
    test('TestDataFactory creates mock detected objects', () {
      ...
      expect(detectedObject.imageLabels, isEmpty); // Default empty list
      expect(detectedObject.hasImageLabels, isFalse);
    });

    test('DetectedObject image labeling support', () {
      // Create a detected object with image labels
      final imageLabels = [
        const ImageLabel(text: 'bottle', confidence: 0.95, index: 0),
        const ImageLabel(text: 'plastic', confidence: 0.87, index: 1),
        const ImageLabel(text: 'container', confidence: 0.72, index: 2),
      ];
      ...
      // Test mergeWithLabels method
      final mergedObject = detectedObject.mergeWithLabels(imageLabels);

      expect(mergedObject.imageLabels, hasLength(3));
      ...
      // Test debug string
      expect(mergedObject.imageLabelsDebugString, contains('bottle (95.0%)'));
      expect(mergedObject.imageLabelsDebugString, contains('plastic (87.0%)'));

      // Ensure original object is unchanged
      expect(detectedObject.imageLabels, isEmpty);
      expect(detectedObject.hasImageLabels, isFalse);
    });
    ...
    test('Mock leaderboard entries can be created', () {
      final entries = List.generate(
        3,
        (index) => TestDataFactory.createMockLeaderboardEntry(
          username: 'user$index',
