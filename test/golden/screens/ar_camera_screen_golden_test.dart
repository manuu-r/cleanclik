...
void main() {
  group('ARCameraScreen Golden Tests', () {
    ...
    testGoldens('ARCameraScreen renders correctly in light theme', (
      tester,
    ) async {
      await loadAppFonts();
      ...
      final widget = baseTest.createTestWidget(
        ...
        overrides: [
          ...
          detectedObjectsProvider.overrideWith(
            (ref) => [
              {'category': 'recycle', 'confidence': 0.95},
            ],
          ),
        ],
      );
    ...
    testGoldens('ARCameraScreen renders correctly in dark theme', (
      tester,
    ) async {
      await loadAppFonts();
      ...
      await tester.pumpWidgetBuilder(widget, surfaceSize: const Size(375, 812));
      await tester.pumpAndSettle();
    ...
    testGoldens('ARCameraScreen QR scanning mode', (tester) async {
      ...
      final widget = baseTest.createTestWidget(
        ...
        overrides: [
          cameraStateProvider.overrideWith(
            (ref) => {'mode': 'qr_scanning', 'status': 'active'},
          ),
          authStateProvider.overrideWith(
            (ref) => const AsyncValue.data({
              'status': 'signed_in',
    ...
    testGoldens('ARCameraScreen tablet layout', (tester) async {
      ...
      final widget = baseTest.createTestWidget(
        ...
        overrides: [
          ...
          detectedObjectsProvider.overrideWith(
            (ref) => [
              {'category': 'organic', 'confidence': 0.87},
            ],
          ),
        ],
      );
    ...
    testGoldens('ARCameraScreen with accessibility features', (tester) async {
      ...
      final widget = baseTest.createTestWidget(
        ...
        overrides: [
          ...
          detectedObjectsProvider.overrideWith(
            (ref) => [
              {'category': 'ewaste', 'confidence': 0.92},
            ],
          ),
        ],
      );
