...
class HandCoordinateTransformer {
  ...
  static CoordinateTransformationResult transformHandCenter(
    ...
    try {
      ...
      final screenCoordinates = _applyTransformation(handCenter, screenSize, imageSize);
      
      // Validate coordinates
      final isValid = _validateCoordinates(screenCoordinates, screenSize);
      
      final debugInfo = {
        'normalized_center': '${handCenter.dx.toStringAsFixed(3)},${handCenter.dy.toStringAsFixed(3)}',
        'screen_size': '${screenSize.width.toInt()}x${screenSize.height.toInt()}',
        'image_size': '${imageSize.width.toInt()}x${imageSize.height.toInt()}',
        'final_coordinates': '${screenCoordinates.dx.toInt()},${screenCoordinates.dy.toInt()}',
  ...
  static List<Offset> transformAllLandmarks(
    ...
    return normalizedLandmarks.map((landmark) => 
        _applyTransformation(landmark, screenSize, imageSize)).toList();
  }
  ...
  static Offset _calculateHandCenterFromPalm(List<Offset> landmarks) {
    ...
    if (landmarks.length < 21) {
      print('⚠️ [COORD] Invalid landmark count: ${landmarks.length}, using fallback');
  ...
  static Offset _applyTransformation(Offset normalizedCoords, Size screenSize, Size imageSize) {
...
class CoordinateDebugger {
  ...
  static void logRawLandmarks(List<Offset> landmarks, String handId) {
    ...
    if (landmarks.length >= 21) {
      ...
      print('   Wrist: ${wrist.dx.toStringAsFixed(3)},${wrist.dy.toStringAsFixed(3)}');
      print('   Thumb tip: ${thumbTip.dx.toStringAsFixed(3)},${thumbTip.dy.toStringAsFixed(3)}');
      print('   Index tip: ${indexTip.dx.toStringAsFixed(3)},${indexTip.dy.toStringAsFixed(3)}');
    } else {
      print('⚠️ [COORD] Invalid landmark count for $handId: ${landmarks.length}');
    }
  }
  ...
  static void logTransformationSteps(
    ...
    print('   Transformed: ${transformed.dx.toInt()},${transformed.dy.toInt()}');
    print('   Screen: ${screenSize.width.toInt()}x${screenSize.height.toInt()}');
  }
  ...
  static void logValidationResult(
    ...
    print('$status [COORD] Validation for $handId: ${coordinates.dx.toInt()},${coordinates.dy.toInt()} - $reason');
  }
