...
class CategoryStatsDatabaseService extends DatabaseService<CategoryStats> {
  ...
  Future<DatabaseResult<Map<String, CategoryStats>>> getUserCategoryStatsMap(
    String userId,
  ) async {
    return await executeWithRetry(
  ...
  Future<DatabaseResult<Map<String, int>>> getUserCategoryCountsMap(
    String userId,
  ) async {
    return await executeWithRetry(
  ...
  Future<DatabaseResult<Map<String, int>>> getTotalUserStats(
    String userId,
  ) async {
    return await executeWithRetry(
  ...
  Future<DatabaseResult<List<CategoryStats>>> batchUpdateCategoryStats(
    ...
    categoryUpdates, // category -> {itemCount, totalPoints}
  ) async {
    return await executeWithRetry(
      ...
      action: () async {
        ...
        final response = await client
            .from(tableName)
            .upsert(dataList, onConflict: 'user_id,category')
            .select();
