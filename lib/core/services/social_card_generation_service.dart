...
import 'motivational_message_service.dart';
...
class SocialCardGenerationService {
  ...
  Future<File> generateCard({
    ...
    try {
      print('🎨 SocialCardGenerationService: Generating ${template.displayName} card for ${platform.displayName}');
      
      final dimensions = _platformOptimizer.getDimensions(platform);
      print('📐 SocialCardGenerationService: Card dimensions - ${dimensions.width}x${dimensions.height}');
      
      final widget = _buildCardWidget(template, data, platform);
      print('🔧 SocialCardGenerationService: Widget built successfully');
      
      final filename = 'vibesweep_${template.name}_${platform.name}_${DateTime.now().millisecondsSinceEpoch}.png';
      ...
      print('✅ SocialCardGenerationService: Card generated successfully at: ${file.path}');
      return file;
    } catch (e, stackTrace) {
      ...
      throw SocialCardGenerationException(
        message: 'Failed to generate ${template.displayName} card for ${platform.displayName}',
        originalError: e,
      );
    }
  }
  ...
  Future<CardData> aggregateUserData(WidgetRef ref) async {
    try {
      ...
      print('👤 SocialCardGenerationService: User found - ${currentUser.username}, Points: ${currentUser.totalPoints}');
  ...
  Widget _buildCardWidget(CardTemplate template, CardData data, SocialPlatform platform) {
    switch (template) {
      case CardTemplate.achievement:
        ...
        child: Column(
          ...
          children: [
            ...
            if (data.recentBadges.isNotEmpty) ...[
              ...
              Wrap(
                spacing: 12,
                children: data.recentBadges.take(3).map((badge) => Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  decoration: BoxDecoration(
                    color: Colors.white.withOpacity(0.2),
...
final socialCardGenerationServiceProvider = Provider<SocialCardGenerationService>((ref) {
