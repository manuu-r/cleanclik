...
class LeaderboardDatabaseService extends DatabaseService<LeaderboardEntry> {
  ...
  Future<DatabaseResult<LeaderboardPage>> getLeaderboardPage({
    ...
    return await executeWithRetry(
      ...
      action: () async {
        ...
        debugPrint('🏆 [DB] Building leaderboard query with filter: $filter, sort: $sort');
        ...
        final entries = response.map<LeaderboardEntry>((row) {
          return LeaderboardEntry.fromSupabase(row, currentUserId: currentUserId);
        }).toList();
  ...
  Future<DatabaseResult<LeaderboardPage>> getUserRankContext({
    ...
    return await executeWithRetry(
      ...
      action: () async {
        ...
        final startRank = (userRank - contextSize).clamp(1, double.infinity).toInt();
  ...
  Future<DatabaseResult<List<LeaderboardEntry>>> searchUsers({
    ...
    return await executeWithRetry(
      ...
      action: () async {
        final searchQuery = client.from('leaderboard').select('''
          id,
          username,
          total_points,
          level,
  ...
  Stream<List<LeaderboardEntry>> subscribeToLeaderboard({
    ...
    return client
        ...
        .order('rank', ascending: true)
        .limit(limit)
        .map((data) => data.map<LeaderboardEntry>((row) {
              return LeaderboardEntry.fromSupabase(row, currentUserId: currentUserId);
            }).toList());
  }
    ...
    return await executeWithRetry(
      ...
      action: () async {
        ...
        final activeUsersQuery = client
            .from('leaderboard')
            .select('id')
            .gte('last_active_at', DateTime.now().subtract(const Duration(days: 7)).toIso8601String());
  ...
  /// Get user's rank
  Future<DatabaseResult<int>> _getUserRank(String userId, LeaderboardFilter filter) async {
    return await executeWithRetry(
