...
class QRBinService {
  ...
  static BinInfo? parseQRCode(String qrData) {
    ...
    try {
      ...
      print('✅ [$_logTag] Successfully parsed bin: ${binInfo.binId} (${binInfo.category.id})');
      _logBinScan(binInfo);
  ...
  static void _logBinScan(BinInfo binInfo) {
    if (kDebugMode) {
      debugPrint('📊 [$_logTag] Bin scanned: ${binInfo.binId} at ${binInfo.locationName ?? 'Unknown location'}');
    }
  }
  ...
  static void _logParsingError(String qrData, String error) {
    if (kDebugMode) {
      ...
      debugPrint('📊 [$_logTag] QR data: ${qrData.length > 200 ? qrData.substring(0, 200) + '...' : qrData}');
    }
  }
  ...
  static String getErrorMessage(String qrData) {
    ...
    try {
      ...
      final categoryString = data['category'] as String?;
      if (categoryString == null || WasteCategory.fromString(categoryString) == null) {
