...
class CameraResourceManager {
  ...
  Future<CameraController> requestCamera(CameraMode mode) async {
    ...
    developer.log('Camera request: $mode (ID: $requestId)', name: 'CameraResourceManager');
          ...
          await _cameraLock.synchronized(() async {
            ...
            if (_activeController != null) {
              ...
              request.completer.completeError(
                const CameraInitializationException('Failed to initialize camera controller'),
              );
            }
          });
        } catch (e) {
  ...
  Future<void> _switchToMode(CameraMode mode) async {
    if (_currentMode == mode && _activeController?.value.isInitialized == true) {
      developer.log('Already in mode $mode, skipping switch', name: 'CameraResourceManager');
      return;
    }
    ...
    try {
      ...
      developer.log('Camera mode switch failed: $e', name: 'CameraResourceManager');
      rethrow;
    } finally {
      _notifyStateChange();
    }
  ...
  Future<void> _initializeCameraForMode(CameraMode mode) async {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        developer.log('Camera initialized successfully for mode: $mode', name: 'CameraResourceManager');
        return;
      } catch (e) {
        retryCount++;
        developer.log('Camera init attempt $retryCount failed: $e', name: 'CameraResourceManager');
  ...
  Future<void> _disposeCurrentCamera() async {
    if (_activeController != null) {
      developer.log('Disposing camera controller', name: 'CameraResourceManager');
      try {
        await _activeController!.dispose();
      } catch (e) {
        developer.log('Error disposing camera: $e', name: 'CameraResourceManager');
      }
      _activeController = null;
    }
    _cancelIdleTimer();
  ...
  void _notifyStateChange() {
    if (!_stateController.isClosed) {
      ...
      developer.log('Camera state changed: ${state.mode} - ${state.status}', name: 'CameraResourceManager');
    }
  }
  ...
  void _resetIdleTimer() {
    ...
    _idleTimer = Timer(_idleTimeout, () {
      developer.log('Camera idle timeout reached', name: 'CameraResourceManager');
  ...
  Future<void> _optimizeForLowMemory() async {
    ...
    developer.log('Optimizing camera for low memory', name: 'CameraResourceManager');
  }
