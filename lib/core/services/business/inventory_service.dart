...
class InventoryService extends _$InventoryService {
    ...
    try {
      ...
      _realtimeSubscription = _supabase
          ...
          .listen(
            ...
            onError: (error) {
              // Check if this is a logout-related error (expected)
              final errorString = error.toString().toLowerCase();
              if (errorString.contains('timedout') || 
                  errorString.contains('channelerror') ||
                  errorString.contains('unauthorized') ||
                  errorString.contains('1006')) {
                print('🔄 [$_logTag] Real-time subscription disconnected (expected during logout): $error');
              } else {
  ...
  Future<void> dispose() async {
    try {
      ...
      // Cancel real-time subscription gracefully
      if (_realtimeSubscription != null) {
        print('🔄 [$_logTag] Cancelling real-time subscription...');
        await _realtimeSubscription!.cancel();
        _realtimeSubscription = null;
        print('✅ [$_logTag] Real-time subscription cancelled');
      }
      ...
      print('❌ [$_logTag] Error during disposal: $e');
      // Still clear the subscription reference even if cancellation fails
      _realtimeSubscription = null;
    }
  }
  ...
  void cleanupOnLogout() {
    try {
      print('🔄 [$_logTag] Cleaning up on user logout...');
      
      // Cancel real-time subscription immediately to prevent timeout errors
      if (_realtimeSubscription != null) {
        _realtimeSubscription!.cancel();
        _realtimeSubscription = null;
        print('✅ [$_logTag] Real-time subscription cleaned up for logout');
      }
    } catch (e) {
      print('❌ [$_logTag] Error during logout cleanup: $e');
      _realtimeSubscription = null;
    }
  }
}
