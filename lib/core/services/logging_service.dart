...
class LoggingService {
  ...
  void setLogLevel(LogLevel level) {
    _currentLevel = level;
    _log(LogLevel.info, LogCategory.general, 'Logging level set to ${level.name}');
  }
  ...
  void _log(LogLevel level, LogCategory category, String message, [Object? error, StackTrace? stackTrace]) {
    ...
    switch (level) {
      ...
      case LogLevel.warning:
        developer.log(fullMessage, name: category.prefix, level: 900, error: error, stackTrace: stackTrace);
        break;
      case LogLevel.error:
        developer.log(fullMessage, name: category.prefix, level: 1000, error: error, stackTrace: stackTrace);
        break;
      case LogLevel.none:
        break;
    }
  ...
  void debug(LogCategory category, String message) => _log(LogLevel.debug, category, message);
  void info(LogCategory category, String message) => _log(LogLevel.info, category, message);
  void warning(LogCategory category, String message, [Object? error, StackTrace? stackTrace]) => 
      _log(LogLevel.warning, category, message, error, stackTrace);
  void error(LogCategory category, String message, [Object? error, StackTrace? stackTrace]) => 
  ...
  void initializeDefaults() {
    ...
    setRateLimit(LogCategory.ar, const RateLimitConfig(
      window: Duration(seconds: 1),
      maxMessages: 2,
    ));
    
    setRateLimit(LogCategory.ml, const RateLimitConfig(
      window: Duration(seconds: 1),
      maxMessages: 2,
    ));
    
    setRateLimit(LogCategory.ui, const RateLimitConfig(
      window: Duration(seconds: 1),
      maxMessages: 3,
    ));
    
    setRateLimit(LogCategory.tracking, const RateLimitConfig(
      window: Duration(seconds: 2),
      maxMessages: 5,
    ));
    
    info(LogCategory.general, 'LoggingService initialized with default rate limits');
  }
}
