...
class DeepLinkService {
  ...
  Future<void> _handleEmailVerification(Uri uri) async {
    try {
      ...
      final result = await _userService.verifyEmailWithToken(token, email);

      if (result.success && result.user != null) {
        debugPrint('Email verification successful for: ${result.user!.email}');
        _showMessage('Email verified successfully! Welcome to CleanClik!');

        // Navigate to home screen after successful verification
        _navigate('/');
      } else {
        debugPrint('Email verification failed: ${result.error}');
        ...
        } else if (result.error?.contains('invalid') == true) {
          _showMessage(
        ...
        // Navigate to email verification screen to allow retry
        _navigate('/email-verification', extra: {'email': email});
      }
    } catch (e) {
...
DeepLinkService deepLinkService(Ref ref) {
  ...
  final service = DeepLinkService(userService);
