...
/// Service for managing bin locations using LocalStorageService
class BinLocationService {
  ...
  final StreamController<List<BinLocation>> _binsController =
      StreamController<List<BinLocation>>.broadcast();

  /// Stream of bin location updates
  Stream<List<BinLocation>> get binsStream => _binsController.stream;

  BinLocationService(this._storageService) {
    _initialize();
  }

  /// Initialize the service using LocalStorageService
  ...
  Future<bool> addBin(BinLocation bin) async {
    // Get current bins to check for duplicates
    await getAllBins();
    ...
    final existingBin = _cachedBins.where((existing) {
      ...
      return distanceInMeters <= 10; // 10 meter threshold for duplicates
    }).firstOrNull;

    if (existingBin != null) {
      print(
        '🗑️ [BIN_SERVICE] Duplicate bin detected within 10m of existing bin: ${existingBin.name}',
      );
  ...
  Future<bool> removeBin(String binId) async {
    ...
    final binToRemove = _cachedBins.where((bin) => bin.id == binId).firstOrNull;
    if (binToRemove == null) {
      return false; // Bin not found
    }
  ...
  Future<BinLocation?> findNearestBin(
    ...
    if (category != null) {
      candidateBins = candidateBins
          .where((bin) => bin.category == category)
          .toList();
    }
  ...
  BinLocation? createBinFromQRData(String qrData) {
    try {
      ...
      // Generate unique ID
      final id =
          'bin_${DateTime.now().millisecondsSinceEpoch}_${Random().nextInt(1000)}';
  ...
  Future<Map<String, dynamic>> getStorageStats() async {
    await getAllBins(); // Ensure bins are loaded
  ...
  Future<void> _loadBinsFromStorage() async {
    try {
      // Use LocalStorageService for local storage
      final binsDataString = await _storageService.getString(_storageKey);
      final lastUpdateDataString = await _storageService.getString(_lastUpdateKey);

      if (binsDataString != null) {
        try {
          final binsData = jsonDecode(binsDataString) as Map<String, dynamic>;
          if (binsData['bins'] != null) {
            final binsList = binsData['bins'] as List<dynamic>;
            _cachedBins = binsList
                .map((json) => BinLocation.fromJson(json as Map<String, dynamic>))
                .toList();

            print(
              '📦 [BIN_SERVICE] Loaded ${_cachedBins.length} bins from local storage',
            );
          }
        } catch (e) {
          print('❌ [BIN_SERVICE] Failed to parse cached bins: $e');
        }
      }

      if (lastUpdateDataString != null) {
        try {
          final lastUpdateData = jsonDecode(lastUpdateDataString) as Map<String, dynamic>;
          if (lastUpdateData['timestamp'] != null) {
            _lastUpdate = DateTime.parse(lastUpdateData['timestamp'] as String);
          }
        } catch (e) {
          print('❌ [BIN_SERVICE] Failed to parse last update: $e');
        }
      }

      // Notify listeners if we have data
      if (_cachedBins.isNotEmpty) {
        _binsController.add(List.from(_cachedBins));
      }
    } catch (e) {
      print('❌ [BIN_SERVICE] Error loading bins from local storage: $e');
      _cachedBins = [];
    }
  }
  ...
  Future<void> _saveBinsToStorage() async {
    try {
      // Use LocalStorageService for local storage
      final binsData = {
        'bins': _cachedBins.map((bin) => bin.toJson()).toList(),
      };
      await _storageService.setString(_storageKey, jsonEncode(binsData));

      _lastUpdate = DateTime.now();
      final lastUpdateData = {'timestamp': _lastUpdate!.toIso8601String()};
      await _storageService.setString(_lastUpdateKey, jsonEncode(lastUpdateData));

      print(
        '💾 [BIN_SERVICE] Saved ${_cachedBins.length} bins to local storage',
      );
    } catch (e) {
      print('❌ [BIN_SERVICE] Error saving bins to local storage: $e');
    }
  }
  ...
  int _calculateStorageSize() {
    try {
      final binsJson = jsonEncode(
        _cachedBins.map((bin) => bin.toJson()).toList(),
      );
...
final binLocationServiceProvider = FutureProvider<BinLocationService>((
  ref,
) async {
  final storageService = LocalStorageService.instance;
  await storageService.initialize();
  final service = BinLocationService(storageService);
...
final binLocationsStreamProvider = StreamProvider<List<BinLocation>>((
  ref,
) async* {
  final binServiceAsync = ref.watch(binLocationServiceProvider);
  await for (final asyncValue in Stream.value(binServiceAsync)) {
    if (asyncValue.hasValue) {
      yield* asyncValue.value!.binsStream;
    }
  }
});
