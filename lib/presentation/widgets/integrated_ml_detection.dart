...
import '../../core/models/detected_object.dart' as app_models;
import '../../core/providers/camera_provider.dart';
import '../../core/services/camera_resource_manager.dart';
...
class IntegratedMLDetection extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<IntegratedMLDetection> createState() => _IntegratedMLDetectionState();
}
...
class _IntegratedMLDetectionState extends ConsumerState<IntegratedMLDetection> {
  ...
  Future<void> _startImageStream() async {
    if (_cameraController?.value.isInitialized != true || _isImageStreamActive) {
      return;
    }
  ...
  mlkit.InputImage? _convertCameraImage(CameraImage image) {
    try {
      ...
      final mlkit.InputImageRotation imageRotation = mlkit.InputImageRotationValue.fromRawValue(
        camera.sensorOrientation,
      ) ?? mlkit.InputImageRotation.rotation0deg;

      final mlkit.InputImageFormat inputImageFormat = mlkit.InputImageFormatValue.fromRawValue(
        image.format.raw,
  ...
  List<app_models.DetectedObject> _convertMLKitObjects(List<mlkit.DetectedObject> mlkitObjects) {
  ...
  Widget build(BuildContext context) {
    ...
    if (cameraState.status == CameraStatus.error || _errorMessage != null) {
      return _buildErrorView(cameraState.errorMessage ?? _errorMessage ?? 'ML detection error occurred');
    }
