...
class _DebugOverlayWidgetState extends State<DebugOverlayWidget> {
  ...
  void _updateMetrics() {
    setState(() {
      ...
      _recentMetrics = logger.getRecentMetrics(since: const Duration(seconds: 5));
      
      // Calculate average frame time from recent metrics
      final frameTimes = _recentMetrics
          .where((m) => m.operation.contains('frame') || m.operation.contains('detection'))
          .map((m) => m.duration.inMilliseconds)
          .toList();
      
      if (frameTimes.isNotEmpty) {
        _frameTime = (frameTimes.reduce((a, b) => a + b) / frameTimes.length).round();
      }
    });
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildMetricRow('Frame Time', '${_frameTime}ms', _getFrameTimeColor()),
          _buildMetricRow('Memory', '${_memoryUsage.toStringAsFixed(1)}MB', Colors.blue),
  ...
  Widget _buildOperationRow(PerformanceMetric metric) {
    final color = metric.duration.inMilliseconds > 100 ? Colors.red : Colors.white54;
