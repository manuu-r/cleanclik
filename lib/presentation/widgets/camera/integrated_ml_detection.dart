...
import 'package:cleanclik/core/models/camera_models.dart' as app_models;
...
class _IntegratedMLDetectionState extends ConsumerState<IntegratedMLDetection> {
  ...
  Widget build(BuildContext context) {
    ...
    if (cameraState.status == app_models.CameraStatus.error ||
        _errorMessage != null) {
    ...
    if (cameraState.status == app_models.CameraStatus.initializing ||
        cameraState.status == app_models.CameraStatus.switching) {
      return _buildLoadingView();
    }

    // Handle camera not ready
    if (!cameraState.isReady ||
        cameraState.mode != app_models.CameraMode.mlDetection) {
      return _buildLoadingView();
    }
