...
class _BaseMaterialOverlayState extends State<BaseMaterialOverlay>
    ...
    return Material(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          ...
          final exitOpacity = _isExiting
              ? (1.0 - _exitAnimation.value).clamp(0.0, 1.0)
              : 1.0;
          final combinedOpacity = (entranceOpacity * exitOpacity).clamp(
            0.0,
            1.0,
          );
  ...
  Widget _buildBackgroundScrim(BuildContext context, double opacity) {
    final backgroundColor =
        widget.backgroundColor ?? Colors.black.withValues(alpha: 0.6 * opacity);
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final breathingValue =
            1.0 + (intensity * (0.5 + 0.5 * (animation.value * 2 - 1).abs()));
        return Transform.scale(scale: breathingValue, child: child);
      },
    );
  }
