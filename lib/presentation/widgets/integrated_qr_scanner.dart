...
/// QR Scanner widget that integrates with the camera resource manager
class IntegratedQRScanner extends ConsumerStatefulWidget {
  final Function(String) onQRScanned;
  final VoidCallback? onClose;
  final bool showOverlay;

  const IntegratedQRScanner({
    super.key,
  ...
  @override
  ConsumerState<IntegratedQRScanner> createState() => _IntegratedQRScannerState();
}

class _IntegratedQRScannerState extends ConsumerState<IntegratedQRScanner>
  ...
  /// Initialize camera through resource manager
  Future<void> _initializeCamera() async {
    try {
      // Use Future.microtask to avoid modifying provider during build
      await Future.microtask(() async {
        await ref.read(cameraNotifierProvider.notifier).switchToQRMode();
      });
    } catch (e) {
      debugPrint('Failed to initialize QR camera: $e');
    }
  }
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    debugPrint('QR scanner initialized successfully');
  }
  
  void _onQRScanned(String qrData) {
    if (!_isScanning) return;
    
    debugPrint('QR code detected: ${qrData.length} characters');
  ...
  void _onPermissionSet(QRViewController ctrl, bool hasPermission) {
    debugPrint('QR Scanner camera permission: $hasPermission');
    if (!hasPermission) {
  ...
  Widget build(BuildContext context) {
    ...
    if (cameraState.status == CameraStatus.error) {
      return _buildErrorView(cameraState.errorMessage ?? 'Camera error occurred');
    }
    ...
    if (!cameraState.isReady || cameraState.mode != CameraMode.qrScanning) {
      return _buildLoadingView();
    }

    return widget.showOverlay ? _buildOverlayView() : _buildScannerView();
  }
                ...
                return Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: NeonColors.electricGreen.withValues(alpha: 0.5),
                          blurRadius: 4,
                          spreadRadius: 1,
                        ),
                      ],
      ...
      child: Column(
        ...
        children: [
          Icon(
            ...
            color: _isScanning ? NeonColors.oceanBlue : NeonColors.electricGreen,
            size: 32,
          ),
          const SizedBox(height: UIConstants.spacing2),
          Text(
