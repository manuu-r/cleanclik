...
class CoverageAnalyzer {
  ...
  Future<void> _generateOverallReport(
    Map<String, FileCoverage> coverage,
  ) async {
    final totalLines = coverage.values.fold(
      0,
      (sum, file) => sum + file.linesFound,
    );
    final totalHit = coverage.values.fold(
      0,
      (sum, file) => sum + file.linesHit,
    );
    final overallPercentage = totalLines > 0
        ? (totalHit / totalLines) * 100
        : 0.0;
  ...
  Future<void> _generateCategoryReports(
    Map<String, FileCoverage> coverage,
  ) async {
    ...
    for (final category in pathPatterns.keys) {
      ...
      final categoryFiles = coverage.entries
          .where(
            (entry) => patterns.any((pattern) => entry.key.contains(pattern)),
          )
      ...
      final totalHit = categoryFiles.fold(
        0,
        (sum, file) => sum + file.linesHit,
      );
      ...
      print(
        '$status $category: ${percentage.toStringAsFixed(2)}% (${categoryFiles.length} files)',
      );

      // Show poorly covered files
      final poorlyCovered =
          categoryFiles.where((file) => file.percentage < threshold).toList()
  ...
  Future<void> _generateUncoveredFilesReport(
    Map<String, FileCoverage> coverage,
  ) async {
    ...
    final uncoveredFiles =
        coverage.values.where((file) => file.percentage == 0.0).toList()
  ...
  Future<void> _generateRiverpodProviderReport() async {
    ...
    for (final file in providerFiles) {
      ...
      final testPath =
          'test/unit/${relativePath.replaceFirst('.dart', '_test.dart')}';
  ...
  Future<void> _checkThresholds(Map<String, FileCoverage> coverage) async {
    ...
    print(
      '${overallPassed ? '✅' : '❌'} Overall: ${overallPercentage.toStringAsFixed(2)}% (required: ${thresholds['overall']}%)',
    );
    ...
    for (final category in pathPatterns.keys) {
      ...
      allPassed &= passed;
      print(
        '${passed ? '✅' : '❌'} $category: ${percentage.toStringAsFixed(2)}% (required: ${threshold}%)',
      );
    }
